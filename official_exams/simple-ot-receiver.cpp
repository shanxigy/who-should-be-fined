...
main(int argc, char** argv)
{
	...
	crypto *m_cCrypt = new crypto(m_nSecParam, (uint8_t*)m_cConstSeed[1]);
  CLock *glock = new CLock();

	std::unique_ptr<CSocket> m_Socket = Connect(address, port);
	if (!m_Socket) {
	...
	auto m_cBaseOT = new SimpleOT(m_cCrypt, m_eFType);
